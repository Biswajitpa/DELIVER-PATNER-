...
export const createOrder = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  if (
    ...
    address.location.coordinates[0] === undefined ||
    address.location.coordinates[1] === undefined
  ) {
    return res.status(400).json({
  ...
  const restaurant = firstCartItem.restaurantId as IRestaurant;
  const restaurantId = restaurant._id;

  console.log("RESTAURANT ID FROM CART =>", restaurantId);
  console.log("RESTAURANT DATA =>", restaurant);
  console.log("AUTO LOCATION =>", restaurant?.autoLocation);
  console.log("COORDINATES =>", restaurant?.autoLocation?.coordinates);
  ...
  if (
    ...
    return res.status(400).json({
      message: "Restaurant location is unavailable",
    });
  }

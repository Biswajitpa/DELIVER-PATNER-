...
export const addRestraunt = TryCatch(async (req, res) => {
    ...
    try {
        ...
        console.log("Restaurant image upload error:", error?.response?.data || error.message);
        return res.status(500).json({
            message: error?.response?.data?.message || "Failed to upload restaurant image",
        });
    }
...
export const updateRestaurant = TryCatch(async (req, res) => {
    ...
    const restaurant = await Restaurant.findOneAndUpdate({ ownerId: req.user._id }, { name, description }, { new: true });
    if (!restaurant) {
        return res.status(404).json({
            message: "Restaurant not found",
        });
